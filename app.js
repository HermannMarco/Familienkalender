...
function expandEvent(event, rangeStart, rangeEnd) {
  if (!event.recurring || event.recurring.type === 'none') {
    ...
    // Multi-day: one all-day instance per day in range
    const instances = [];
    let cur = new Date(Math.max(start.getTime(), rangeStart.getTime()));
    const endMs = Math.min(end.getTime(), rangeEnd.getTime());
    while (cur.getTime() <= endMs) {
      instances.push({ ...event, date: fmt(cur), startTime: null, endTime: null, _multiDay: true });
      cur = addDays(cur, 1);
    }
    return instances;
  }
...
function getEventColor(event) {
  if (event.type === 'geburtstag') return '#f59e0b';
...
function renderMonthCell(ds, dayNum, otherMonth, events, todayStr) {
  ...
  for (let i = 0; i < Math.min(sorted.length, MAX); i++) {
    ...
    const label = ev.type === 'geburtstag' ? `🎂 ${ev.title}` : ev.title;
    const typeCls = ev.type === 'geburtstag' ? ' birthday' : '';
...
function renderWeekView() {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    alldayDays += `<div class="wg-day" style="min-height:${evs.length?28:4}px">${evs.map(ev => {
      const col = getEventColor(ev);
      return `<div class="cell-event-chip" style="background:${col};margin:1px;cursor:pointer" onclick="App.openEventDetail('${ev.id}','${ev.date}')">${ev.type==='geburtstag'?'🎂 ':''}${ev.title}</div>`;
    }).join('')}</div>`;
  }
  const alldayEl = document.getElementById('week-allday');
  if (hasAllday) {
    alldayEl.innerHTML = `<div class="wg-wrap"><div class="wg-times"></div><div class="wg-days">${alldayDays}</div></div>`;
    alldayEl.classList.remove('hidden');
  } else {
    alldayEl.classList.add('hidden');
  }

  // Time grid: flex layout, each day is a full-height column
  let timesHtml = '';
  for (let h = 0; h < 24; h++) {
  ...
  for (let day = 0; day < 7; day++) {
    ...
    let colHtml = '';
    for (let h = 0; h < 24; h++) colHtml += `<div class="wg-hour"></div>`;
    ...
    for (const ev of dayEvents) {
      ...
      const col    = getEventColor(ev);
      colHtml += `<div class="event-block" style="background:${col};top:${top}px;height:${height}px" onclick="App.openEventDetail('${ev.id}','${ev.date}')"><span>${ev.title}</span><span class="eb-time">${ev.startTime}${ev.endTime?'–'+ev.endTime:''}</span></div>`;
    }

    if (ds === todayStr) {
      const n = new Date();
      colHtml += `<div class="wg-now" style="top:${n.getHours()*H + n.getMinutes()/60*H}px"><div class="now-dot"></div></div>`;
    }
    daysHtml += `<div class="wg-day">${colHtml}</div>`;
  }

  setHTML('week-grid',
    `<div class="wg-wrap"><div class="wg-times">${timesHtml}</div><div class="wg-days">${daysHtml}</div></div>`);
...
function renderDayView() {
  ...
  alldayEl.innerHTML = allDay.length > 0
    ? allDay.map(ev => {
        const col = getEventColor(ev);
        return `<div class="cell-event-chip" style="background:${col};margin:2px;font-size:.8rem;cursor:pointer" onclick="App.openEventDetail('${ev.id}','${ev.date}')">${ev.type==='geburtstag'?'🎂 ':''}${ev.title}</div>`;
      }).join('')
    : '';
  ...
  const nowD = new Date();
  if (fmt(nowD) === ds) {
    colHtml += `<div class="wg-now" style="top:${nowD.getHours()*H + nowD.getMinutes()/60*H}px"><div class="now-dot"></div></div>`;
  }

  setHTML('day-grid',
    `<div class="wg-wrap"><div class="wg-times">${timesHtml}</div><div class="wg-days"><div class="wg-day">${colHtml}</div></div></div>`);
...
function renderEventCard(ev, showDate = false) {
  ...
  return `<div class="event-card" onclick="App.openEventDetail('${ev.id}','${ev.date}')">
    ...
    <div class="event-card-body">
      <div class="event-card-title">${ev.type==='geburtstag'?'🎂 ':''}${ev.title}</div>
...
function openEditEvent(eventId, dateStr) {
  ...
  if (endDateEl2) endDateEl2.value = ev.endDate || '';
...
async function saveEvent() {
  ...
  const data = {
    ...
    title,
    date: dateVal,
    endDate: (endDateVal && endDateVal > dateVal) ? endDateVal : null,
...
function openEventDetail(id, dateStr) {
  ...
  let html = `<div class="detail-title">${ev.type==='geburtstag'?'🎂 ':''}${ev.title}</div>`;
